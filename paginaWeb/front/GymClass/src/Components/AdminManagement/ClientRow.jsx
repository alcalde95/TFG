...
export const ClientRow = ({ client }) => {
    ...
    return (
        <tr key={client.id} className='border-gray-500 border-[1px] '>
            <td>{client.email}</td>
            <td>{client.role}</td>
            <td><button className="bg-teal-500 m-1 w-20 h-10 border-2 border-teal-500 text-white p-1 rounded-md mr-2 hover:bg-teal-400 hover:border-white  shadow-[2px_2px_5px_0px] shadow-gray-500"
                ...
                </button>
                {
                    showEdit
                        ? <form name="editUserForm"
                            ...
                            onSubmit={(e) => handleSubmit({ e, email: client.email, defaultPassword: client.password, defaultRole: client.role })}
                        >
                            <input name="password"
